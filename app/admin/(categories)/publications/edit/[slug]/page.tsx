...
function AddPublication({ params }: { params: { slug: string } }) {
  const { slug } = params;
  ...
  useEffect(() => {
    ...
    const { title, slug, content } = publication;
    // @ts-ignore
  ...
  async function handlePatch(body: any) {
    const res = await fetch(`/api/publication/${slug}`, {
      method: "PATCH",
      headers: {
        ...
        <div className="flex flex-wrap -mx-3">
          <div className="w-full px-3 mb-2">
            ...
            <input
              ...
              type="text"
              name="slug"
              placeholder="e.g. title-of-your-publication-as-slug"
              ref={slugRef}
            />
          </div>
        </div>
